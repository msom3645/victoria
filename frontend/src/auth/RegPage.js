...
const RegPage = () => {
    ...
    const writeDb = async() => {
        try{
            const {data} = await axios.post('/api/auth/reg-model', {user, email, pass})
            setOk(true)
            login()
            timer(()=> {h('/')}, 2000)
        }catch(e){
    ...
    const mail = async() => {
        setLoad(true)
        try{
            const {data} = await axios.post('/api/auth/reg', {user, email, text:'To authenticate, please use the following One Time Password (OTP):', subj:'Email Verification'})
            setRand(data.rand)
            setLoad(false) 
        }catch(e){
            e.response && e.response.data.message? setErr(e.response.data.message):
