...
const ResetPass = () => {
    ...
    const writeDb = async(e) => {
        ...
        if(pass === repass){
            try{
                setLoad(true)
            const {data} = await axios.put('/api/auth/user', {email, pass})
            login()
        }catch(e){
            e.response && e.response.data.message? setErr(e.response.data.message):
            setErr('Server Error, Try later!')
    ...
    const mail = async() => {
        setLoad(true)
        try{
            const {data} = await axios.post('/api/auth/reset-pass', {email, text:'To reset password, please use the following One Time Password (OTP):', subj:'ResetPassword'})
            setRand(data.rand)
            setLoad(false) 
        }catch(e){
            e.response && e.response.data.message? setErr(e.response.data.message):
