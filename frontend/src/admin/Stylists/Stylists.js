...
const Stylists = () => {
  ...
  useEffect(()=>{
      const fetch = async()=>{
            try{
                const {data} = await axios.get('http://localhost:5000/api/admin/stylists', {headers:{ Authorization: `Bearer ${user.token}` }})
                setList(data)
            }catch(e){
...
{list?.map(e => (
    <div className='adm-img-r f fcenter' key={e._id}>
            ...
            <img src={'/uploads/stylists/'+e.img} alt={e.img}/>
            {confirm.sts && confirm.id === e?._id && <div className='adm-conf'>
                <h4 style={{color:'var(--cream)'}}>Are You Sure?</h4>
                <div>
                <button onClick={()=>removeItem('StylistsModel',  `http://localhost:5000/api/admin/models-utils/${e._id}`, setLoad, setErr, setMsg, e.imgPath)}>Yes</button>
