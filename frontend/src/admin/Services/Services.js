...
const Services = () => {
  ...
  useEffect(()=>{
      const fetch = async()=>{
            try{
                const {data} = await axios.get('http://localhost:5000/api/admin/services', {headers:{ Authorization: `Bearer ${user.token}` }})
                setList(data)
            }catch(e){
    ...
    return (               
        ...
        <AdminPanel>
        <Back/>
        <Link className='adm-add-link f fcenter' to='/admin-panel/services/add'>Add New Service <Plus color={'var(--spray)'}/> </Link>

{list.map(e => (
    <div className='adm-img-r f fcenter' key={e._id}>
            <div className='adm-btns'>
            <Link to={`/admin-panel/services/${e?._id}`}>
            ...
            <img src={`/uploads/services/${e.img}`} alt='no image'/>
            {confirm.sts && confirm.id === e?._id && <div className='adm-conf'>
                <h4 style={{color:'var(--cream)'}}>Are You Sure?</h4>
                <div>
                <button onClick={()=>removeItem('ServicesModel',  `http://localhost:5000/api/admin/models-utils/${e._id}`, setLoad, setErr, setMsg, e.imgPath)}>Yes</button>
