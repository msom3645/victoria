...
const Phone = () => {
    ...
    useEffect(()=>{
        const fetch = async()=>{
            setLoad(true)
              try{
                  const {data} = await axios.get('/api/admin/phone', {headers:{ Authorization: `Bearer ${user.token}` }})
                    setList(data)
                    setPhone(data.phone)
      ...
      const send = async(e) => {
        e.preventDefault() 
                setLoad(true)
                try{
                    const {data} = await axios.put('/api/admin/phone', {phone, phone2}, {headers:{ Authorization: `Bearer ${user.token}` }})
                    setMsg(data.message)
                    setLoad(false)
                }catch(e){
