...
const Slider = () => {
  ...
  useEffect(()=>{
      const fetch = async()=>{
            try{
                const {data} = await axios.get('http://localhost:5000/api/admin/slider', {headers:{ Authorization: `Bearer ${user.token}` }})
                setList(data)
            }catch(e){
...
{list.map(e => (
        <div className='adm-img-r f fcenter' key={e._id}>
            ...
            {confirm.sts && confirm.id === e?._id && <div className='adm-conf'>
                ...
                <div>
                <button onClick={()=>removeItem('SliderModel',  `http://localhost:5000/api/admin/models-utils/${e._id}`, setLoad, setErr, setMsg, e.imgPath)}>Yes</button>
