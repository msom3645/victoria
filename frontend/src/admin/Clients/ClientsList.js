...
const ClientsList = () => {
    ...
    useEffect(()=>{
        const fetch = async() => {
            try{
                const {data} = await axios.post('http://localhost:5000/api/admin/book-clients', {salon}, {headers:{ Authorization: `Bearer ${user.token}` }})
                setNames(data)
            }catch(e){
            ...
            {names.map(el=> (
                <div key={el._id} className='cl-cards f fc'>
                    ...
                    <div style={{gap: '14px'}} className='f fc'>
                        ...
                        <button onClick={()=>removeItem('BookModel',  `http://localhost:5000/api/admin/models-utils/${el._id}`, setLoad, setErr, setMsg)   } className='cl-yes-btn f fcenter'>YES</button>
