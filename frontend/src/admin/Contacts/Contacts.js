...
const Contacts = () => {
  ...
  useEffect(()=>{
    ...
    return (               
        ...
        <AdminPanel>
        <Back/>
        <Link className='adm-add-link f fcenter' to='/admin-panel/contacts/add'>Add New Contact <span className='addpl'><Plus color={'var(--spray)'}/></span> </Link>
        
{list.map(e => (
        <div className='adm-img-r f fcenter' key={e._id}>
            <div className='adm-btns'>
            <Link to={`/admin-panel/contacts/${e?._id}`}>
            ...
            <img src={`/uploads/contacts/${e.img}`} alt='no image'/>
            {confirm.sts && confirm.id === e?._id && <div className='adm-conf'>
                <h4 style={{color:'var(--cream)'}}>Are You Sure?</h4>
                <div>
                <button onClick={()=>removeItem('ContactsModel',  `http://localhost:5000/api/admin/models-utils/${e._id}`, setLoad, setErr, setMsg, e.imgPath)}>Yes</button>
