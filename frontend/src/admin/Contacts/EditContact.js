      ...
      const send = async(e) => {
        e.preventDefault() 
                ...
                const writeToDB = async(pic) => {
                    const items = {items: {tel, location, period, pic} ,model:'ContactsModel', id}
                    try{
            ...
            const f = (compressed) =>{
                ...
                typeof(img) === 'object'? compressImg(img, setErr, setLoad, 200, f) : writeToDB(img)          
        }

        useEffect(()=>{
            list?.period !== period || list?.tel !== tel || list?.location !== location || preview ? setDisable(false): setDisable(true)     
        },[tel, location, preview,list])
