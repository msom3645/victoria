...
const BookNow = () => {
    ...
    const chooseSalon = async(salon) =>{
        ...
        try{
            const {data} = await axios.post('/api/admin/stylists', {salon:salon.location})
            setStep1(true)
        setSpecs(data)
      }catch(e){
          console.log(e)
    ...
    const findDates = async() => {
        try{
            const {data} = await axios.post('/api/admin/book-dates',{salon, spec})
            setStep1(false)
            setStep2(true)
            setDatesNA(data)
          }catch(e){
    ...
    const bookNow = async () =>{
          try{
            const {data} = await axios.post('/api/admin/book', {name, phone, salon, date:[day, hour], spec, service})
            setBooked(true)
            setStep3(false)
        }catch(e){
