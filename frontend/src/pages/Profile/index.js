...
const Profile = () => {
    ...
    useEffect(()=>{
        const fetch = async() => {
            try{
                const {data} = await axios.post(user.isAdmin? '/api/user/admin-profile': '/api/user/user-profile', {id:user._id}, {headers: {Authorization: `Bearer ${user.token}`}})
                   setProfile(data)
            }catch(e){
...
const edit = async(e) => {
    ...
    setLoad(true)
    try{
        const {data} = await axios.put(user.isAdmin?'/api/user/admin-profile-edit':'/api/user/user-profile-edit', {id:user._id, newEmail}, {headers: {Authorization: `Bearer ${user.token}`}})
        setMsg(data.message)
...
const editPass = async(e) => {
    ...
    setLoad(true)
    try{
        const {data} = await axios.put(user.isAdmin?'/api/user/admin-pass':'/api/user/user-pass', {id:user._id, oldPass, pass: pass1}, {headers: {Authorization: `Bearer ${user.token}`}})
        setMsg(data.message)
        setChangeEmail(false)
        setChangePass(false)
        setLoad(false)
...
const editPic = async(e) => {
    e.preventDefault()
        setLoad(true)
                const writeToDB = async(userPic) => {
                        try{
                            const {data} = await axios.put(user.isAdmin?'/api/user/admin-profile-edit':'/api/user/user-profile-edit', {id:user._id, userPic}, {headers: {Authorization: `Bearer ${user.token}`}})
                            setMsg(data.message)
                            setLoad(false)
                            setChangeImg(false)
                        }catch(e){
                ...
                const f = (compressed) =>{
                    ...
                    typeof(pic) === 'object' && compressImg(pic, setErr, setLoad, 200, f)    
}

const remove = async() => {
    setLoad(true)
    try{
        const {data} = await axios.post(user.isAdmin?'/api/user/admin-profile-edit':'/api/user/user-profile-edit', {id:user._id}, {headers: {Authorization: `Bearer ${user.token}`}})
        setMsg(data.message)
        setLoad(false)
    }catch(e){
        setErr(e.message)
    ...
    return (
        <div className='pr-fr f fc'>
                ...
                <div className='pr-pic-btns f fc fcenter'>
                    ...
                    {!pic && profile?.pic && <button onClick={remove} className='pr-set-btn pr-del f fcenter'><i className="fas fa-trash"></i></button>}
                    {pic && <button onClick={()=>setPic()} className='pr-set-btn pr-remove f fcenter'><i className="fas fa-window-close"></i></button>}
                    {pic && <button onClick={editPic} className='pr-set-btn pr-save f fcenter' >SAVE</button>}
                    </div>
                    ...
                    <button onClick={remove}  className='cl-yes-btn f fcenter' >YES</button>
                ...
                {changeEmail &&
                <form onSubmit={edit} style={{gap:'14px',position:'relative'}} className='pr-pic-btns f fc'>
                ...
                {changePass &&
                <form onSubmit={editPass} style={{gap:'20px'}} className='f fc'>
                    ...
                    {pass2?.length>5 && pass2?.length>5 && pass1 === pass2 && oldPass?.length>0 && <button className='send-btn'>SAVE</button>}
                </form>
            }
            </div>
