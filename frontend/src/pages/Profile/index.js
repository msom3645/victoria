...
const Profile = () => {
    ...
    useEffect(()=>{
        const fetch = async() => {
            try{
                const {data} = await axios.post(user.isAdmin? 'http://localhost:5000/api/user/admin-profile': 'http://localhost:5000/api/user/user-profile', {id:user._id}, {headers: {Authorization: `Bearer ${user.token}`}})
                   setProfile(data)
            }catch(e){
...
const emailBoard = () => {
    ...
    const saveImg = async() => {      
        ...
        const f = async(compressed) => {
            ...
            try{
                const {data} = await axios.post(user.isAdmin? 'http://localhost:5000/api/user/admin-profile-img':'http://localhost:5000/api/user/user-profile-img', formData, {headers: {Authorization: `Bearer ${user.token}`}})
                setMsg(data.message)
                setPic()
                setLoad(false)
            }catch(e){
...
const removeImg = async() => {
    try{
        const {data} = await axios.put(user.isAdmin? 'http://localhost:5000/api/user/admin-profile-img':'http://localhost:5000/api/user/user-profile-img', {imgPath: profile.imgPath, img: profile.img, id: user._id},{headers: {Authorization: `Bearer ${user.token}`}})
        setMsg(data)
        setConfirm(false)
    }catch(e){
        console.log(e)
...
const emailHandler = async(e) => {
    e.preventDefault()
    try{
        const {data} = await axios.put(user.isAdmin?'http://localhost:5000/api/user/admin-profile-email':'http://localhost:5000/api/user/user-profile-email', {id:user._id, newEmail}, {headers: {Authorization: `Bearer ${user.token}`}})
        setEmail('')
        setMsg(data)
    }catch(e){
            console.log(e)
...
const passHandler = async(e) => {
    e.preventDefault()
    try{
        const {data} = await axios.put(user.isAdmin? 'http://localhost:5000/api/user/admin-profile-pass':'http://localhost:5000/api/user/user-profile-pass', {id:user._id, pass:pass1, oldPass}, {headers: {Authorization: `Bearer ${user.token}`}})
        setMsg(data)
    }catch(e){
